...
import {
  ...
  FiTruck  // Added for shipping charge icon
} from 'react-icons/fi';
...
const SortableGalleryItem = ({
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`relative rounded-lg overflow-hidden shadow-md border-2 cursor-move select-none
    ...
    >
      ...
      <button
        ...
        className="absolute top-1 right-1 bg-red-500 text-white w-5 h-5 rounded-full flex items-center justify-center hover:bg-red-600 z-20"
      ...
      {/* COLOR SELECT ✅ */}
      <div
        className="p-1 bg-gray-50 border-t"
        onPointerDown={(e) => e.stopPropagation()} // 🔑 IMPORTANT
      >
        <select
          value={image.color || ""}
...
const EditProductPage = () => {
  ...
  const [productData, setProductData] = useState({
    ...
    productTag: '',
    variants: [],
    shippingCharge: '', // NEW FIELD: shipping charge
    existingImages: [],
    existingVideo: null,
    ...
    const fetchProductData = async () => {
      ...
      try {
        ...
        setProductData({
          ...
          shippingCharge:
  productData.shippingCharge !== undefined
    ? productData.shippingCharge
    : 0,// NEW FIELD: shipping charge
  ...
  return {
    ...
    color: imageObj.color || '',
      ...
      const productToUpdate = {
        ...
        // ✅ SHIPPING CHARGE (NEW FIELD)
 shippingCharge:
  productData.shippingCharge !== '' &&
  productData.shippingCharge !== null
    ? parseFloat(productData.shippingCharge)
    : 0,
  ...
  const resetForm = () => {
    if (existingProduct) {
      ...
      setProductData({
        ...
        shippingCharge: product.shippingCharge ?? 0, // NEW FIELD: shipping charge
        existingImages: normalizedImages,
...
>
  ...
  <SortableContext
    items={galleryFiles.map(img => img.id)}
    ...
    <div className="grid grid-cols-3 md:grid-cols-6 gap-4">
      {galleryFiles.map(image => (
        <SortableGalleryItem
          ...
          handleColorChangeOnImage={handleColorChangeOnImage}
        ...
        />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl  transition-all duration-200 bg-gray-50"
            ...
            {/* SHIPPING CHARGE SECTION (NEW) */}
            <div className="space-y-6">
              <div className="flex items-center space-x-3 mb-4">
                <div className="bg-gradient-to-r from-indigo-100 to-blue-50 p-2 rounded-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">Enter the shipping charge for this product. Leave empty or set to 0 for free shipping.</p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-md mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                  ...
                  <input
                    ...
                    onChange={handleNewVariantChange}
                    placeholder="Size"
                    className="px-3 py-2 border-2 border-gray-200 focus:outline-none focus:ring-0  focus:border-gray-200 rounded-lg bg-gray-50"
                    disabled={isFormDisabled}
                  />
                  <div className="relative">
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      className="w-full pl-8 pr-3 py-2 border-2 focus:outline-none focus:ring-0  focus:border-gray-200 border-gray-200 rounded-lg  bg-gray-50"
                      disabled={isFormDisabled}
                    />
                  </div>
                  <div className="relative">
          ...
          <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-2xl max-w-md w-full p-8 space-y-6 animate-slideUp">
            <div className="text-center">
              ...
              <div className="mt-4 p-3 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border border-emerald-200">
                ...
                {productData.shippingCharge && parseFloat(productData.shippingCharge) > 0 && (
                  <p className="text-sm text-emerald-600 mt-1">
                    ✓ Shipping charge: ₹{parseFloat(productData.shippingCharge).toFixed(2)}
                  </p>
                )}
                {(!productData.shippingCharge || parseFloat(productData.shippingCharge) === 0) && (
                  <p className="text-sm text-emerald-600 mt-1">
